<template>
    <v-row class="mx-8">
        <v-col cols="4">
            <v-label v-if="store.packValues.length > 0" class="text-h4 mb-2">Packs</v-label>
            <PackView v-for="packValue in store.packValues" :pack-value="packValue"/>
        </v-col>
        <v-col cols="8">
            <v-row no-gutters>
                <v-col v-if="store.selectedPack.cardValues != null" cols="12"><v-label class="text-h4 mb-2">Cards</v-label></v-col>
